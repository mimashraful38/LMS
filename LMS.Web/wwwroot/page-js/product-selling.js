...
$('#inputBarCode').typeahead({
    minLength: 1,
    displayText: function (item) {
        return `${item.ProductCode}  ${item.IsSold === true ? ' - Sold' : ' - Stock' }`;
    },
    afterSelect: function (item) {
        this.$element[0].value = item.ProductCode
    },
    source: function (request, result) {
        $.ajax({
            url: "/Selling/FindProducSearch",
            data: { code: request },
...
const validation = function () {
    ...
    var customerType = $("input[name='IsIndividual']:checked").val();
    ...
    if (customerType === "false") {
        ...
        if (customerName === "" || customerPhone === "" ) {
            customerError.innerText = 'Add customer Name Or Phone No!';
            return false;
        }
...
const onSellSubmitClicked = function (evt) {
    ...
    const body = {
        ...
        customerType: customerType === 'true' ? 'Regular' :'Register',
...
function appendInfo(item) {
    ...
    const html = `<h5 class="badge badge-pill badge-success">${name}</h5>
        <span class="badge badge-pill badge-primary">Address: <span id="prevDue">${item.CustomerAddress}</span></span>
        <span class="badge badge-pill badge-dark">Phone No: <span id="prevDue">${item.PhonePrimary}</span></span>
...
function checkDueLimit() {
    ...
    if (customerType === "false") {
      ...
      // const prevDue = +formPayment.querySelector('#prevDue').textContent || 0;
        ...
        if (due > dueLimit) {
            customerError.innerText = `Plase Pay Full Amount ${due}/-`;
            return false;
        }
